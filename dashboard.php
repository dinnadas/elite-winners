...
if (isset($_POST['reset_test_data']) && isset($_SESSION['admin_id'])) {
    try {
        ...
        $pdo->commit();
        $_SESSION['success_message'] = "All data wiped! Revenue starts at $0.00";
    } catch (Exception $e) {
        $pdo->rollBack();
...
try {
    $stmt = $pdo->query("
        ...
        WHERE o.payment_status = 'paid'
    ");
    $totalSold = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
} catch (PDOException $e) {
    $totalSold = 'Error';
}
...
try {
    $stmt = $pdo->query("
        ...
        WHERE o.payment_status = 'paid'
    ");
    $totalRevenue = number_format((float)$stmt->fetchColumn(), 2);
} catch (Exception $e) {
    $totalRevenue = '0.00';
}
...
try {
    ...
    foreach ($salesData as $row) {
        $salesLabels[] = $row['title'];
        $salesValues[] = (int)$row['units_sold'];
    }
} catch (PDOException $e) {
    $salesLabels = ['Error'];
    $salesValues = [0];
...
try {
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM bookings WHERE preferred_date = ?");
        $stmt->execute([$dateStr]);
        $weeklyBookings[] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
    }
} catch (PDOException $e) {
...
try {
    ...
    foreach ($sessionData as $i => $row) {
        $sessionLabels[] = $row['session_type'] ?: 'Unknown';
        ...
        <aside class="sidebar hidden lg:block lg:w-64 bg-sidebar-bg text-white">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-6 overflow-y-auto">
                    <ul class="space-y-2">
                        <li>
                            <a href="dashboard.php" class="flex items-center px-4 py-3 rounded-lg bg-eww-green text-white">
            ...
            <main class="p-6">
                ...
                <div class="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-yellow-800">Clear Test Data</h3>
                            <p class="text-sm text-yellow-700">Remove all test orders, revenue, visitors, and bookings. Real data stays safe.</p>
                        </div>
                        <form method="POST" onsubmit="return confirm('WARNING: This will delete ALL test data. Are you sure?');">
                            <input type="hidden" name="reset_test_data" value="1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center">
                            <div class="p-3 rounded-lg bg-eww-green-light">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-eww-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                            </div>
                            <div class="ml-4">
                                <h2 class="text-sm font-medium text-gray-600">Total Bookings</h2>
                                <p class="text-2xl font-bold"><?php echo htmlspecialchars($totalBookings); ?></p>
                            </div>
                        </div>
                        <div class="mt-4">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center">
                            <div class="p-3 rounded-lg bg-eww-gold-light">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-eww-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                </svg>
                            </div>
                            <div class="ml-4">
                                <h2 class="text-sm font-medium text-gray-600">Site Visitors</h2>
                                <p class="text-2xl font-bold"><?php echo htmlspecialchars($totalVisitors); ?></p>
                            </div>
                        </div>
                        <div class="mt-4">
                            <p class="text-xs text-green-600 font-semibold">
                                <span>+5.3%</span> from last week
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h2 class="text-sm font-medium text-gray-600">Products Sold</h2>
                                <p class="text-2xl font-bold"><?php echo htmlspecialchars($totalSold); ?></p>
                            </div>
                        </div>
                        <div class="mt-4">
                            <p class="text-xs text-green-600 font-semibold">
                                <span>+12.7%</span> from last week
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h2 class="text-sm font-medium text-gray-600">Revenue</h2>
                                <p class="text-2xl font-bold">$<?php echo htmlspecialchars($totalRevenue); ?></p>
                            </div>
                        </div>
                        <div class="mt-4">
                            <p class="text-xs text-green-600 font-semibold">
                                <span>+8.4%</span> from last week
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-8">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <a href="bookings.php" class="text-eww-green text-sm font-medium">View All</a>
                    </div>
                                ...
                                <?php else: ?>
                                    <?php foreach ($recentBookings as $booking): ?>
                                        ...
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-6 w-6">
                                                        <div class="h-6 w-6 rounded-full bg-eww-green-light flex items-center justify-center text-eww-green text-xs font-semibold"><?php echo htmlspecialchars($initials); ?></div>
                                                    </div>
                                                    <div class="ml-3">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars($booking['session_type']); ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars($preferredDate); ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars($booking['age']); ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars($booking['phone'] ?: 'No phone'); ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <button class="text-eww-green hover:text-eww-dark mr-3">Edit</button>
    ...
    <div id="mobile-menu" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute left-0 top-0 bottom-0 w-64 bg-sidebar-bg text-white transform transition-transform duration-300 ease-in-out -translate-x-full" id="mobile-sidebar">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-6 overflow-y-auto">
                    <ul class="space-y-2">
                        <li><a href="dashboard.php" class="flex items-center px-4 py-3 rounded-lg bg-eww-green text-white">Dashboard</a></li>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const bookingsChart = new Chart(bookingsCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: 'rgba(11, 122, 77, 0.1)',
                        borderColor: '#0B7A4D',
                        borderWidth: 2,
                        tension: 0.3,
                        fill: true
            ...
            const sessionChart = new Chart(sessionCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: <?php echo json_encode($sessionValues); ?>,
                        backgroundColor: <?php echo json_encode($sessionBgColors); ?>,
                        borderWidth: 2,
                        borderColor: '#fff'
                    }]
                },
